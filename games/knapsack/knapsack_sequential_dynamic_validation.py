...
class KnapsackSequentialDynamicValidation(gym.Env):
    def __init__(self,env_config):
        self.config = env_config
        self.instances_size = self.config["instances_size"]
        self.dataset_type = self.config["dataset_type"]
        self.dataset_size = 100
        self.dataset = np.load("/home/users/coelho/quantum-computing/QRL/games/knapsack/KP_dataset.npy", allow_pickle=True).item()
        ...
        for i in range(self.dataset_size):
            self.values.append(list(self.dataset[f"{self.instances_size}"][self.dataset_type][f"{i}"]["values"]))
            self.weights.append(list(self.dataset[f"{self.instances_size}"][self.dataset_type][f"{i}"]["weights"]))
            self.maximum_weight.append(self.dataset[f"{self.instances_size}"][self.dataset_type][f"{i}"]["maximum_weight"])
        ...
        self.optimal_actions = [list(sol_str) for sol_str in self.solution_str]
        self.optimal_actions = [[int(a) for a in optimal_action] for optimal_action in self.optimal_actions]
        self.optimal_rewards = [self.calculate_reward(optimal_action,timestep = i) for i,optimal_action in enumerate(self.optimal_actions)]
    ...
    def broke_constraint(self,action,timestep):
        weight = sum(action[i]*self.weights[timestep][i] for i in range(self.instances_size))
        return round(weight,2) > self.maximum_weight[timestep]
...
if __name__ == "__main__":
    ...
    env = KnapsackSequentialDynamicValidation(config)
